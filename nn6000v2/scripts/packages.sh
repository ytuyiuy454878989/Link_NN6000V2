...
FEEDS_SCRIPT="${BUILD_DIR}/scripts/feeds"

# 封装写入配置函数，防重复
add_config() {
    local cfg_line="$1"
    local cfg_file="${BUILD_DIR}/mydiffconfig"
    if ! grep -Fxq "$cfg_line" "$cfg_file" 2>/dev/null; then
        echo "$cfg_line" >> "$cfg_file"
    fi
}

# 判断feeds.conf是否存在，不存在则新建
if [ ! -f "$FEEDS_CONF" ]; then
    touch "$FEEDS_CONF"
fi

# 精确判断源是否已存在，不存在再追加
if ! grep -Fxq "$KENZO_FEED" "$FEEDS_CONF"; then
    echo "$KENZO_FEED" >> "$FEEDS_CONF"
fi

# 校验feeds脚本是否存在
if [ ! -f "$FEEDS_SCRIPT" ]; then
    echo "错误：未找到OpenWrt feeds脚本 $FEEDS_SCRIPT，请检查BUILD_DIR路径配置" >&2
    exit 1
fi
...
# 写入编译勾选配置
#add_config "CONFIG_PACKAGE_luci-app-xunlei=y"
#add_config "CONFIG_PACKAGE_luci-app-linkease=y"
